...
<div class="container-fluid px-4">
    <div class="card mb-4">
        <div class="card-header">
            ...
            Edit Quota Allocation
        </div>
        <div class="card-body">
            <form action="{{ route('organisation.quota-allocations.update', [$organisation->slug, $quotaAllocation->id]) }}" method="POST">
                @csrf
                @method('PATCH')
                ...
                <div class="mb-3">
                    ...
                    <select name="species_id" id="species_id" class="form-select @error('species_id') is-invalid @enderror" required>
                        ...
                        @foreach($species as $s)
                            <option value="{{ $s->id }}" {{ (old('species_id', $quotaAllocation->species_id) == $s->id) ? 'selected' : '' }}>
                                {{ $s->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('hunting_quota') is-invalid @enderror" 
                                   id="hunting_quota" name="hunting_quota" 
                                   value="{{ old('hunting_quota', $quotaAllocation->hunting_quota) }}" 
                                   min="0" required>
                            @error('hunting_quota')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('rational_killing_quota') is-invalid @enderror" 
                                   ...
                                   value="{{ old('rational_killing_quota', $quotaAllocation->rational_killing_quota) }}" 
                                   min="0" required>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('start_date') is-invalid @enderror" 
                                   ...
                                   value="{{ old('start_date', $quotaAllocation->start_date->format('Y-m-d')) }}" 
                                   required>
                            @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('end_date') is-invalid @enderror" 
                                   ...
                                   value="{{ old('end_date', $quotaAllocation->end_date->format('Y-m-d')) }}" 
                                   required>
                            @error('end_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" 
                              id="notes" name="notes" rows="3">{{ old('notes', $quotaAllocation->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    <button type="submit" class="btn btn-primary">Update Quota Allocation</button>
