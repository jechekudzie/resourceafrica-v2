    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3 mb-0">Add New Dynamic Field</h1>
            ...
            <div class="card">
                <div class="card-header bg-success text-white">
                    <h5 class="card-title mb-0"><i class="fas fa-plus-circle me-2"></i>Field Details</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('organisation.dynamic-fields.store', $organisation->slug) }}" method="POST">
                        @csrf
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (['select', 'checkbox', 'radio'].includes(fieldTypeSelect.value)) {
            ...
            // If options were previously entered (old values), restore them
            @if (old('options'))
                @foreach (old('options') as $index => $option)
                    addOption('{{ $option['option_value'] ?? '' }}', '{{ $option['option_label'] ?? '' }}');
                @endforeach
            @else
