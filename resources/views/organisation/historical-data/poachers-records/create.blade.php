...
@section('title')
Regional CBNRM - {{ $organisation->name }} - Add Poachers Record
@endsection
...
<div class="card">
    <div class="card-header bg-white">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0">
                    <i class="fas fa-plus-circle me-1 text-primary"></i>Add Poachers Record
                </h5>
                <p class="mb-0 text-muted small">Create a new poachers arrest record</p>
            </div>
            <div>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('poachers_records.store', $organisation->slug) }}" method="POST">
            @csrf
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('arrested') is-invalid @enderror" 
                                    id="arrested" name="arrested" value="{{ old('arrested', 0) }}" 
                                    min="0" required>
                                @error('arrested')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('bailed') is-invalid @enderror" 
                                    id="bailed" name="bailed" value="{{ old('bailed', 0) }}" 
                                    min="0" required>
                                @error('bailed')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('sentenced') is-invalid @enderror" 
                                    id="sentenced" name="sentenced" value="{{ old('sentenced', 0) }}" 
                                    min="0" required>
                                @error('sentenced')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
