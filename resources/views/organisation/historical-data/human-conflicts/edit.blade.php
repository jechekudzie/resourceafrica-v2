...
@section('title')
Regional CBNRM - {{ $organisation->name }} - Edit Human Conflict Record
@endsection
...
<div class="card">
    <div class="card-header bg-white">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0">
                    <i class="fas fa-user-injured me-1 text-primary"></i>Edit Human Conflict Record
                </h5>
                <p class="mb-0 text-muted small">Update historical human casualties from wildlife conflicts</p>
            </div>
            <div>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('human_conflict_records.update', [$organisation->slug, $humanConflict]) }}" method="POST">
            @csrf
            @method('PATCH')
            <div class="row g-4">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="period" id="period" class="form-select @error('period') is-invalid @enderror" required>
                            ...
                            @for ($year = date('Y'); $year >= 1990; $year--)
                                <option value="{{ $year }}" {{ old('period', $humanConflict->period) == $year ? 'selected' : '' }}>{{ $year }}</option>
                            @endfor
                        </select>
                        @error('period')
                    ...
                    <div class="form-group">
                        ...
                        <select name="species_id" id="species_id" class="form-select @error('species_id') is-invalid @enderror" required>
                            ...
                            @foreach ($species as $specie)
                                <option value="{{ $specie->id }}" {{ old('species_id', $humanConflict->species_id) == $specie->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select name="gender_id" id="gender_id" class="form-select @error('gender_id') is-invalid @enderror" required>
                            ...
                            @foreach ($genders as $gender)
                                <option value="{{ $gender->id }}" {{ old('gender_id', $humanConflict->gender_id) == $gender->id ? 'selected' : '' }}>
                                    {{ $gender->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="number" name="deaths" id="deaths" class="form-control @error('deaths') is-invalid @enderror" 
                            value="{{ old('deaths', $humanConflict->deaths) }}" min="0" required>
                        @error('deaths')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="number" name="injured" id="injured" class="form-control @error('injured') is-invalid @enderror" 
                            value="{{ old('injured', $humanConflict->injured) }}" min="0" required>
                        @error('injured')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
