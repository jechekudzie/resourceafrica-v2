...
@section('title')
Regional CBNRM - {{ $organisation->name }} - Add Poaching Record
@endsection
...
<div class="card">
    <div class="card-header bg-white">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0">
                    <i class="fas fa-plus-circle me-1 text-primary"></i>Add Poaching Record
                </h5>
                <p class="mb-0 text-muted small">Create a new poaching incident record</p>
            </div>
            <div>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('poaching_records.store', $organisation->slug) }}" method="POST">
            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('poaching_method_id') is-invalid @enderror" 
                                    ...
                                    @foreach($poachingMethods as $method)
                                        <option value="{{ $method->id }}" {{ old('poaching_method_id') == $method->id ? 'selected' : '' }}>
                                            {{ $method->name }}
                                        </option>
                                    @endforeach
                                </select>
