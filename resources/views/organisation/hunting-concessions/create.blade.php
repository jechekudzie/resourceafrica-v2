...
@section('title', 'Add New Hunting Concession')
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Add New Hunting Concession</h5>
                ...
                <div class="card-body">
                    <form action="{{ route('organisation.hunting-concessions.store', $organisation->slug ?? '') }}" method="POST">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('hectarage') is-invalid @enderror" 
                                           id="hectarage" name="hectarage" value="{{ old('hectarage') }}">
                                    @error('hectarage')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('safari_id') is-invalid @enderror" id="safari_id" name="safari_id">
                                        ...
                                        @foreach ($safaris as $safari)
                                            <option value="{{ $safari->id }}" {{ old('safari_id') == $safari->id ? 'selected' : '' }}>{{ $safari->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('safari_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="text-end mt-3">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Save Concession
                            </button>
                        </div>
                    </form>
                </div>
